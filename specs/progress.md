...
- **`env` vs `outputs` naming:** Applications use `spec.env` (what the container consumes); Resources use `spec.outputs` (what they publish). The asymmetry reflects the directional difference: outputs are produced, env is consumed.
  - **(Revised — `specs/021-resource-env-output-split/`):** Resources now declare BOTH `spec.env` (runtime config, like an Application, plus `generated`) AND `spec.outputs` (a pure export allowlist: `name` + optional `template`; no value/valueFrom/generated). `outputs` is a strict allowlist — consumers read only listed keys. Resources are first-class consumers (env `valueFrom` may reference other manifests' exports). Acceptance: SC-001 env/output separation; SC-002 un-exported keys unreadable; SC-003 generated secret stays private by default; SC-004 old-shape manifests rejected with migration error; SC-005 same-name migration preserves the generated secret.
