...
Path format: `vault:<project>/<environment>/<secret>`. The project component can be the project's **name** (e.g. `shrine-test`), its slug (auto-generated by Infisical, e.g. `shrine-test-2jdn`), or its UUID — the plugin resolves any of them to the underlying ID via a one-time `/api/v1/workspace` lookup. Environment is the env slug (`prod`/`staging`/`dev` by default).
